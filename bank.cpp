...
void PrintClientRecordLineForTransactions(sClient Client)
{
    cout << "| " << setw(15) << left << Client.AccountNumber;
    cout << "| " << setw(40) << left << Client.Name;
    cout << "| " << setw(12) << left << Client.AccountBalance;
...
sClient Deposit(sClient& Client, double deposit)
{
    sClient C;
    ...
    C.AccountBalance = deposit + Client.AccountBalance;
    cout << "\n\nClient deposit Successfully,Client has " << C.AccountBalance << ".";
...
sClient Withdraw(sClient& Client, double withdraw)
{
    sClient C;
    C.AccountNumber = Client.AccountNumber;
    C.PinCode = Client.PinCode;
    ...
    C.AccountBalance = (-withdraw) + Client.AccountBalance;
    cout << "\n\nClient withdraw Successfully,Client has " << C.AccountBalance << ".";
    ...
    if (FindClientByAccountNumber(AccountNumber, vClients, Client))
    {
        ...
        if (Answer == 'y' || Answer == 'Y')
        {
            for (sClient& C : vClients)
            {
                if (C.AccountNumber == AccountNumber)
                {
                    C = Deposit(Client, depositAmount);
...
bool WithdrawByAccountNumber(string AccountNumber, vector<sClient>& vClients)
{
    ...
    if (FindClientByAccountNumber(AccountNumber, vClients, Client))
    {
        ...
        while (withdrawAmount > Client.AccountBalance)
        {
            ...
            cout << "\nPlease enter anouther withdraw amount? ";
            cin >> withdrawAmount;
        }
        cout << "\n\nAre you sure you want update this client? y/n ? ";
        cin >> Answer;
        if (Answer == 'y' || Answer == 'Y')
        {
            for (sClient& C : vClients)
            {
                if (C.AccountNumber == AccountNumber)
                {
                    C = Withdraw(Client, withdrawAmount);
...
void ShowTotalBalancesScreen()
{
    ...
    if (vClients.size() == 0)
        ...
        for (sClient Client : vClients)
        {
            ...
            PrintClientRecordLineForTransactions(Client);
...
void ShowTransactionsScreen()
{
    ...
    PerfromTransactionsMenueOption((enTransactionsMenueOptions)ReadTranactionsMenueOption());
}
